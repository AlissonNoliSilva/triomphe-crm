# PRD — Novo CRM Triomphe (v2, unificado)

**Backend Django + Frontend React (arquitetura de referência: `triomphe-whatsapp`)**

Este documento é a fusão de:
- **PARTE A — Visão de Produto (PRD):** contexto, objetivos, escopo, requisitos, arquitetura, migração, KPIs e riscos.
- **PARTE B — Especificação Funcional do Sistema Atual:** levantamento completo do CRM Odoo em produção. **Tudo o que está na Parte B deve existir no novo CRM (paridade obrigatória)**, salvo decisão explícita em contrário.
- **PARTE C — Melhorias e Novas Funcionalidades:** o que o novo CRM deve fazer *melhor* ou *de novo* face ao atual (inclui o motor de recomendação de imóveis para compradores).

> Data: 2026-07-20 · Fonte da Parte B: leitura direta do código em `Triomphe addons/`.

---
---

# PARTE A — VISÃO DE PRODUTO

## A1. Contexto

O CRM atual e o website formam um ecossistema Odoo 8 legado com forte acoplamento entre:
- captação de leads/prospectos (scraper OLX/Imovirtual, website, WhatsApp, operação interna);
- gestão de compradores e proprietários;
- gestão de imóveis, visitas e propostas;
- processo de venda e área financeira (comissões, atos, minutas);
- automações de comunicação (SMS/email) e gamificação por pontos;
- regras de acesso por perfil/equipa/agência/carteira.

Este PRD define os requisitos para construir um novo CRM moderno, desacoplado, auditável e seguro, **preservando 100% das regras de negócio do sistema atual** (Parte B) e evoluindo-o com as melhorias da Parte C.

## A2. Objetivo do Produto

Construir um CRM imobiliário unificado para operar os funis de:
- **proprietários** (prospecção → angariação → venda/arrendamento);
- **compradores** (contacto → visita → proposta → processo);
- **investidores** (área restrita Invest);

com automações configuráveis, motor de recomendação comprador↔imóvel, integrações externas controladas, governança de acesso e observabilidade ponta a ponta.

## A3. Escopo

### In
- Intake omnichannel de leads (website, operação interna, importação, API, scraper, WhatsApp).
- Todas as entidades e fluxos da Parte B: Prospecto, Contacto (comprador/proprietário), Imóvel, Visitas (3 tipos), Proposta, Processo de Venda, Comissões/Atos/Minutas, RH (pontos, escalas, tarefas, recrutamento), notificações, relatórios.
- Motor de automações por regra (estado + tempo + canal + consentimento) substituindo os ~40 crons atuais.
- Motor de recomendação e matching comprador↔imóvel (Parte C).
- Gestão de acesso por perfil, equipa, agência e carteira (modelo `responsible_users` da Parte B §3).
- Área de cliente e website público (fase posterior, com API desacoplada desde o dia 1).
- Integrações: SMS (Usendit), email transacional, Habitar, Idealista, portais XML, Triomphe Chat (WhatsApp), GeoIP.

### Out (fase 1)
- Reescrita completa do front-end público do website (a API pública nasce na fase 1; o site migra em fase própria).
- BI corporativo avançado fora dos indicadores operacionais.
- Substituição de todos os sistemas externos legados no dia 1 (migração progressiva).

## A4. Stakeholders e Personas

| Persona | Necessidade principal |
|---|---|
| Direção comercial | Visão consolidada de funil, conversão, produtividade, indicadores semanais por agência |
| Gestão operacional (CO) | Distribuição de leads, fila de trabalho diária, SLAs de contacto, controlo de qualidade |
| Agentes/avaliadores | Carteira própria (leads/imóveis/compradores), agenda de visitas, propostas, comissões, pontos/ranking |
| Coordenadoras/Gestoras | Supervisão da equipa, aprovações, relatórios semanais, escalas |
| Financeiro (Sale Process) | Processos, atos (CPCV/escritura), fichas de comissão, movimentos |
| Marketing | Performance de canais/campanhas, publicação em portais, qualidade de anúncios |
| Backoffice/Compliance | Auditoria, segurança de documentos (PIN), RGPD, qualidade de dados |
| Cliente final (proprietário/comprador/investidor) | Área de cliente: acompanhar processo, responder a propostas, receber recomendações |
| TI/Produto | Evolução contínua, integrações, configuração sem deploy |

## A5. Problemas do legado a resolver

- Regras críticas espalhadas em módulos de 4.000+ linhas, difíceis de manter.
- **IDs hard-coded** (employees 21/22/60/249/265, users 7/8, cargos 1/3/4/6, empresas por nome, distritos por ID, estágios por índice).
- **Credenciais no código** (Usendit, GeoIP, Idealista).
- Rotas públicas com gravações privilegiadas e proteção desigual contra abuso.
- Automações implementadas como crons imperativos, sem visibilidade nem configuração.
- Baixa rastreabilidade de eventos de funil; matching de compradores computado on-the-fly sem persistência.
- Alto acoplamento entre website e modelos internos.
- Complexidade de permissões com risco de abertura excessiva.
- UI Odoo 8 lenta e datada; sem mobile.

## A6. Visão Funcional (macro)

```
Captação (site / scraper / WhatsApp / manual / API)
  → Prospecto + fila de teleprospecção (CO)
    → Visita de Angariação (equipa → agente → confirmação → resultado + QC)
      → Imóvel Angariado (documentos → fotos/marca d'água → publicação site+portais)
        → Compradores (contactos, pesquisas, RECOMENDAÇÕES automáticas)
          → Visita Comercial → Proposta (contrapropostas, validação por código)
            → Processo de Venda (KYC, aprovações) → Atos (CPCV/Reforço/Escritura/Renda)
              → Comissões (cálculo automático, confirmação do agente, pagamento)
```
Transversais: notificações internas, SMS/email com log, pontos/gamificação, escalas, tarefas, relatórios semanais, dashboards, área de cliente, auditoria.

## A7. Modelo de Domínio (entidades novas ↔ legado)

| Novo | Legado (Odoo) | Notas |
|---|---|---|
| `Prospect` | `crm.lead` | Prospecção de proprietários; snapshot imutável ao concluir |
| `Interaction` | `crm.phonecall` + `res.partner.conversation` | Unificar chamadas/SMS/email/WhatsApp/presencial num só histórico |
| `Contact` | `res.partner` | Papéis: comprador, proprietário, investidor, agência externa; KYC |
| `BuyerSearch` | `res.partner.prospection.message` (pesquisa) | Critérios de pesquisa persistidos → motor de recomendação |
| `BuyerInquiry` + `InquiryResponse` | mensagem + respostas | Contactos recebidos e seguimento |
| `Property` | `product.template/product` | Imóvel com estágios, negócios, documentos, media |
| `PropertyBusiness` | `product.business` | Venda/Arrendamento/Troca + histórico de preços |
| `Visit` (3 subtipos) | `calendar.event` / `evaluation.visit` / `commercial.visit` | Angariação, avaliação, comercial + relatórios + QC |
| `Proposal` + `ProposalMovement` | `sale.proposal` + `proposal.movement` | Contrapropostas com histórico |
| `SaleProcess` + `ProcessParticipant` | `sale.process` + participantes | KYC completo, aprovação em 3 níveis |
| `CommissionSheet` | `sale.commissions` | Fórmula da Parte B §10.1; congelamento após revisão |
| `Act` + `Minute`/`Clause` | `sale.acts` + `sale.minute` | CPCV/Escritura/Reforço/Renda/Adenda |
| `MoneyHandling` | `money.handling` | Movimentos por agência/agente |
| `Employee`, `Team`, `PointsCategory`, `PointsEntry`, `Shift` (escala), `Task`, `Availability`, `RecruitmentProspect` | modelos hr.* | Gamificação, escalas, recrutamento |
| `Notification`, `MessageLog` | user.notification + triomphe.sms/email.message | Centro de notificações + log de envios |
| `Recommendation` | *(novo)* | Par comprador↔imóvel com score, estado e feedback (Parte C) |
| `AutomationRule`, `Template` | crons + triomphe.*.template | Motor declarativo (Parte C) |
| `AccessPolicy`, `AuditEvent` | record rules + chatter | RBAC + trilha de auditoria |
| `DocumentVault` + `DocumentAccessLog` | document_security | PIN/janela de acesso + logs |

## A8. Requisitos Funcionais (síntese — detalhe na Parte B)

- **RF-01 Intake unificado:** website, operação, importação, API, scraper, WhatsApp; payload normalizado (source, channel, intent, UTM, consent, locale, IP, UA); devolve `lead_id`.
- **RF-02 Deduplicação e merge:** chave composta (email, telefone, documento, advertID+origem, referência de imóvel); merge manual/automático com histórico; nunca perder dados de origem.
- **RF-03 Funil de proprietário:** estados e transições da Parte B §5 (draft → pending → no_answer/reschedule/sent/received → answer_accepted | disposed) com motivos padronizados e congelamento na conclusão.
- **RF-04 Funil de comprador:** estágios da Parte B §7 com atualização automática, motivos de perda obrigatórios.
- **RF-05 Gestão de imóvel:** ficha completa (Parte B §6), estágios In Prospection → Validar → (Angariação Online) → Angariado → Reservado → Vendido/Arrendado | Cancelado, histórico de preço/estágio, documentos com checklist e cofre por PIN.
- **RF-06 Visitas:** 3 tipos com atribuição em cascata, grelha de disponibilidade, confirmações, resultados por wizard, QC, reagendamento limitado, agenda unificada.
- **RF-07 Propostas e processo:** fluxo completo da Parte B §9 (validação por código, aprovação de comissões divergentes, anexo obrigatório, cancelamento automático de concorrentes, processo com KYC e 3 níveis de aprovação).
- **RF-08 Financeiro:** comissões com fórmula exata da Parte B §10.1, atos com efeitos no imóvel e SMS às partes, minutas com cláusulas, movimentos.
- **RF-09 Interações e comunicação:** registo único de todas as interações; templates versionados PT/EN/FR com flag ativo; log obrigatório de todos os envios; consentimento por canal (whatsapp_opt_out etc.).
- **RF-10 Distribuição e carteira:** fila diária do operador, atribuição manual/automática configurável (distrito × tipo), redistribuição com trilha, histórico de atribuições.
- **RF-11 RH e gamificação:** pontos por categoria/equipa com todas as atribuições automáticas da Parte B §11.2, escalas com abertura por IP, tarefas recorrentes, disponibilidades, recrutamento, relatório semanal de atividade.
- **RF-12 Relatórios:** todos os da Parte B §19 + dashboards por perfil (Parte B §12.1) + indicadores semanais por agência.
- **RF-13 Recomendações:** motor comprador↔imóvel (Parte C, RF-C1).
- **RF-14 Área de cliente:** funcionalidades da Parte B §15 com autenticação moderna.
- **RF-15 Configurabilidade total (Admin Console):** menus, vistas, colunas, campos e **botões de ação** configuráveis por grupo **e por utilizador** (override individual vence o grupo), incluindo os **parâmetros de comportamento de cada botão** (ex.: dias de reagendamento do "Não Atendido") — tudo sem deploy. Detalhe: Parte C, C10.
- **RF-16 Central de Comunicações:** página única que lista **todas** as comunicações automáticas por audiência (prospectos/compradores/proprietários/internos), com estado ativo/inativo, gatilho em linguagem natural, template, estatísticas e **vista de jornada** — documentação viva gerada da própria configuração (impossível ficar desatualizada). Detalhe: Parte C, C2-A.

## A9. Regras de negócio transversais (obrigatórias)

- Toda mudança de estado tem autor, timestamp e origem; estados finais exigem motivo padronizado.
- Todo disparo automático gera log de tentativa e resultado (canal, template, destinatário, estado, detalhe) **visível no histórico do registo** — como o chatter atual.
- Snapshots imutáveis: prospecto concluído (freeze), ficha de comissão revista/paga.
- Propostas não avançam sem consistência de dados nem anexos; visitas comerciais exigem contacto registado.
- Acesso considera papel + agência + equipa + carteira (regras exatas da Parte B §3).
- Ações sensíveis (descarte, cancelamento, reverter proposta, apagar documentos, editar pontos) exigem permissão específica.
- Validações de integridade da Parte B §21 (unicidades, limites, restrições de edição).

## A10. Segurança, Privacidade e Compliance

- Autenticação e autorização centralizadas (RBAC + escopo de dados + permissões finas equivalentes aos ~40 grupos atuais).
- Endpoints públicos: rate limit, captcha, proteção de replay, idempotência, sanitização server-side, validação de ficheiros.
- Segredos em cofre/variáveis de ambiente (eliminar credenciais no código — Usendit, GeoIP, Idealista, JWT).
- Segurança de login: registo de IPs, IPs autorizados por perfil (operadores só de IPs da agência), bloqueio por tentativas, alertas.
- Cofre de documentos com PIN, janelas de acesso temporárias e auditoria de downloads (paridade §13 + melhorias C).
- RGPD: consentimento por canal com versionamento, base legal, direito ao esquecimento/exportação (novo — Parte C).

## A11. Arquitetura alvo

- **Backend Django** (DRF) com apps de domínio: `prospects`, `contacts`, `properties`, `visits`, `proposals`, `finance`, `hr`, `communications`, `recommendations`, `automations`, `accounts` (RBAC), `odoo_sync` (migração/convivência) — espelhando a estrutura já validada no `triomphe-whatsapp` (`accounts`, `billing`, `conversations`, `odoo_sync`, `whatsapp`).
- **Frontend React** (Vite + TanStack, como o triomphe-whatsapp): SPA interna por perfil + design system próprio.
- **Motor de eventos/regras** para automações (Celery + beat para agendamentos; outbox pattern para consistência).
- **API pública separada** (intake de leads, site, área de cliente) desacoplada do domínio interno.
- Base transacional PostgreSQL + trilha de eventos (audit log) + pesquisa full-text.
- Conectores externos por interface com retry/backoff, DLQ, circuit breaker, correlação por `event_id`.
- Observabilidade: logs estruturados, métricas, alertas de falha de automação.

---
---

# PARTE B — ESPECIFICAÇÃO FUNCIONAL DO SISTEMA ATUAL (PARIDADE OBRIGATÓRIA)
## Índice
...
## 22. Recomendações técnicas de implementação
...
# PARTE C — MELHORIAS E NOVAS FUNCIONALIDADES

Tudo o que se segue é **evolução** face ao sistema atual. Prioridades: **[M]** Must (release 1–2), **[S]** Should (release 3), **[C]** Could (roadmap).

## C1. Motor de Recomendação de Imóveis para Compradores **[M]** ⭐

> Exemplo dado pelo negócio: *"ao ter um comprador que tem X valor para investir, ter uma área onde recomenda X imóveis para ele baseado nas características e valores que aceita pagar."*

Hoje já existe um matching rudimentar (campo computado `desired_matched_product_ids` + score `match_value`), mas é calculado on-the-fly, não é persistido, não aparece como área de trabalho e não gera ações. O novo CRM transforma isto num **motor de recomendação de primeira classe**:

### C1.1 Perfil de investimento do comprador
- Cada comprador tem um ou mais **perfis de pesquisa** (`BuyerSearch`): orçamento (valor disposto a investir + tolerância configurável, hoje fixa em +10%), tipo de negócio (compra/arrendamento), tipos de imóvel, tipologias min/max, áreas min/max, nº WC/estacionamento mínimos, condição, comodidades desejadas (com peso "obrigatória" vs "preferida" — evolução do atual interseção/união), localizações multi-nível (distrito/concelho/freguesia/zona, várias linhas), financiamento (pré-aprovado? valor?), horizonte temporal.
- Perfil pode ser criado/atualizado pelo agente, pelo CO, **pelo próprio cliente na área de cliente**, ou inferido automaticamente (ver C1.5).

### C1.2 Área "Recomendações" (novo ecrã)
- **Na ficha do comprador:** separador "Recomendações" com os imóveis ordenados por **score de match** (0–100), mostrando o porquê do score (breakdown: orçamento ✓, concelho ✓, T3 ✓, piscina ✗…), estado de cada recomendação e histórico de envios.
- **Na ficha do imóvel:** separador "Compradores compatíveis" (o inverso) — lista de compradores com score, para o angariador ativar contactos. (Evolução do atual `btn_matched_partner`/`desired_partner_ids`.)
- **Fila de trabalho do agente:** vista "Novas recomendações" com pares comprador↔imóvel ainda não trabalhados, ordenados por score × recência.

### C1.3 Ciclo de vida da recomendação (`Recommendation`)
`gerada → enviada ao cliente → vista pelo cliente → interessado (pediu visita) | rejeitada (com motivo: caro / zona / tipologia / outro) → convertida (visita marcada)`
- Cada transição com timestamp e autor; feedback de rejeição **realimenta o score** (ex.: rejeitou 3 vezes por zona → baixar peso daquela zona).
- Deduplicação: um imóvel nunca é recomendado duas vezes ao mesmo comprador, salvo alteração relevante (ex.: **redução de preço** — que hoje já dispara email — passa a reativar a recomendação com destaque "baixou de X para Y").

### C1.4 Gatilhos automáticos
- **Novo imóvel angariado/publicado** → gera recomendações para todos os compradores compatíveis → notifica agente e (se ativado) envia ao cliente por email/WhatsApp com template e tracking de abertura/clique. (Substitui e expande o atual "save_search_notify" e o email de redução de preço.)
- **Alteração de preço/características** → recalcula matches afetados.
- **Novo perfil de pesquisa** → gera imediatamente o top-N e mostra ao operador ainda durante a chamada.
- **Digest semanal** ao comprador: "5 novos imóveis para si esta semana" (respeitando opt-out e frequência configurável).

### C1.5 Score e inteligência **[S]**
- Score base = evolução da fórmula atual (`match_value`): pesos configuráveis por critério em backoffice (hoje hard-coded: +8 elevador/suite/…, +15 distrito, +20 concelho, +25 freguesia).
- Sinais comportamentais: imóveis visitados/favoritos/clicados no site pelo cliente ajustam o perfil (ex.: só clica em moradias → subir peso de moradias).
- **[C]** Matching semântico com embeddings (descrição do imóvel × notas do comprador) e modelo de propensão (probabilidade de visita/proposta) treinado no histórico de visitas e propostas do CRM.

### C1.6 Recomendação inversa para angariação **[S]**
- Ao criar um **prospecto** (proprietário), mostrar "temos N compradores à procura de um imóvel como este na sua zona" — argumento de angariação para o operador/agente (contagem de BuyerSearch compatíveis com o imóvel do prospecto).

## C2. Motor de automações configurável **[M]**

Substituir os ~40 crons e cadeias hard-coded por regras declaráveis em backoffice:
- **Gatilho** (evento: criação, mudança de estado, tempo decorrido sem X, data agendada) + **condições** (segmento, campos, consentimento) + **ações** (enviar template SMS/email/WhatsApp, criar tarefa/follow-up, mudar estado, notificar, atribuir pontos) + **calendário** (dias úteis, janelas horárias — ex.: só 10h–19h como hoje).
- Cada regra com: ativo/inativo (o "pisco" atual), limite diário (ex.: máx. 20 follow-ups/dia), modo de teste (redirect de destinatário — já existe hoje como parâmetro), log de execuções com sucesso/falha por destinatário (paridade com `external_communication_cron_log`).
- Migrar como regras: SMS de inserção, cadeia de emails do CO (padrão → documentação D+3 → objeção D+5 recorrente), reagendamento noturno de não atendidas (+5 dias 09:00), descarte após 3 emails sem resposta, follow-up de compradores 7 dias, lembrete ao agente 5 dias, arquivamento 180 dias, comunicações pós-visita, SMS de véspera, aniversários, e toda a "Comunicação Exterior".

## C2-A. Central de Comunicações — configuração + documentação viva **[M]** ⭐

> Problema atual (declarado pelo negócio): *"ninguém sabe o que está ativo, o que é enviado e quando é enviado"* — as comunicações estão espalhadas por ~40 crons, templates com "pisco", código hard-coded e o módulo Comunicação Exterior.

**Princípio da solução:** a documentação **não é escrita à mão — é gerada da própria configuração**. Cada comunicação automática só pode existir como regra registada no motor (C2); a Central lê esse registo. Se está na Central, é o que acontece em produção; se não está, não existe. Assim nunca desatualiza.

### C2-A.1 Página "Central de Comunicações"
- **Organização por audiência** (separadores/grupos): **Prospectos** · **Compradores** · **Proprietários** · **Clientes (área online)** · **Internos (colaboradores)**. Dentro de cada audiência, agrupado por etapa do funil (ex.: Prospectos → Inserção / Em prospecção / Qualificados / Pós-visita / Descartados).
- **Cada comunicação é um cartão/linha com:** nome legível; **toggle ativo/inativo** (com permissão e registo de quem alterou e quando); canal (SMS/Email/WhatsApp/Notificação interna) com ícone; **gatilho descrito em linguagem natural gerada automaticamente** (ex.: *"Quando o prospecto é criado"*, *"Todos os dias às 22h, para prospectos não atendidos nesse dia"*, *"3 dias depois do email de documentação, se não houve resposta"*); template usado (nome + pré-visualização PT/EN/FR num clique); condições/exclusões (ex.: "exceto criados por avaliadores", "respeita opt-out WhatsApp"); janela horária e limite diário; **estatísticas**: último disparo, envios/falhas nos últimos 30 dias; responsável pela última alteração.
- **Filtros e pesquisa:** por audiência, canal, estado (ativas/inativas/com falhas), template, texto livre.
- **Indicadores de saúde:** aviso visual em comunicações **inativas que interrompem uma cadeia** (ex.: passo 2 de 3 desligado), com falhas recorrentes, ou com template desativado (o "pisco" atual causava envios silenciosamente não feitos — aqui fica visível).

### C2-A.2 Vista de Jornada (o mapa que falta hoje)
- Por audiência, uma **linha temporal visual** da sequência de comunicações: `D0 SMS inserção → D+3 email documentação → D+5 email objeção (recorrente 5/5 dias) → D+22 descarte automático`, com cada nó a mostrar canal, estado ativo/inativo (nós inativos a cinzento tracejado) e ligação clicável para a regra.
- Ramificações visíveis (ex.: "se atendeu → sai da jornada"; "se sem email → só SMS").
- Exportável para PDF — serve como documentação oficial para formação e auditoria.

### C2-A.3 Simulador e transparência por registo
- **Simulador:** escolher um contacto/prospecto real → ver "que comunicações vai receber e quando" (próximos envios previstos com data/hora e regra de origem). Responde ao "porque é que este cliente recebeu isto?".
- **Na ficha de cada prospecto/contacto:** painel "Comunicações automáticas" com histórico do que já foi enviado (com link para o log) e o que está agendado — o agente/operador vê sem perguntar a ninguém.
- Drill-down de qualquer comunicação para o log real de envios (destinatário, estado, erro).

### C2-A.4 Governança
- Alterar/ativar/desativar exige permissão própria (`gerir comunicações`); tudo auditado (quem, quando, valor anterior→novo).
- Modo de teste por comunicação (redirect de destinatário) com banner visível na Central.
- Histórico de versões de cada regra e de cada template (poder responder "o que estava ativo em março?").

## C3. Comunicação omnicanal unificada **[M]**

- **Timeline única** por contacto/prospecto: chamadas, SMS, emails, WhatsApp, mensagens do site, notas — hoje espalhado por `crm.phonecall`, `res.partner.conversation`, chatter e logs.
- **WhatsApp nativo**: integrar o Triomphe Chat como módulo do novo CRM (mesma base Django) em vez de app separada com JWT; templates aprovados, opt-in/opt-out, atribuição de conversas a agentes.
- Click-to-call com registo automático da chamada e resultado num clique **[S]**; gravação e transcrição **[C]**.
- Caixa de entrada de email sincronizada (IMAP/Graph) com associação automática ao contacto **[S]**.
- Editor de templates com pré-visualização, variáveis tipadas, versões PT/EN/FR lado a lado e estatísticas de abertura/clique por template **[M]**.

## C4. Experiência do agente **[M]**

- **PWA mobile-first** para agentes: agenda do dia, ficha da visita offline, upload de fotos direto da visita (com marca d'água automática no servidor), preenchimento do relatório de visita no local, assinatura do proprietário no ecrã **[M]**.
- Notificações **push** (substituem parte dos SMS internos, reduzindo custo Usendit) **[M]**.
- Pesquisa global full-text (nome, telefone, email, referência, morada) **[M]**.
- Sincronização de calendário Google/Outlook (visitas, atos, escalas) **[S]**.
- "Cockpit" do agente: pendentes (visitas por confirmar, relatórios por preencher, recomendações novas, compradores sem contacto há 5 dias) num só ecrã — hoje isto está disperso em needactions **[M]**.

## C5. Imóveis e publicação **[S]**

- **Feeds automáticos para portais** (Idealista/Imovirtual/Casa Sapo) via API/feed contínuo em vez do wizard XML manual; estado de publicação por portal com erros visíveis (como hoje só existe para a Habitar).
- **Avaliação automática (AVM):** evolução do preço/m² por freguesia — sugerir preço com comparáveis (imóveis semelhantes vendidos/em stock, dias em mercado), gerar o Estudo de Mercado automaticamente.
- Gestão de media: ordenação drag-and-drop, deteção de fotos de baixa qualidade, plantas, vídeo e visita virtual como cidadãos de primeira classe; score de qualidade do anúncio com checklist acionável (evolução do `publication_quality_tips`).
- **Alertas de validade documental:** certificado energético, caderneta, procurações e contratos de mediação com datas de expiração → tarefas automáticas (hoje só existe o lembrete de fim de angariação).
- Geocodificação automática (morada → lat/lng) na gravação.
- **[C]** Geração de descrições do anúncio por IA (PT/EN/FR) a partir das características, com revisão humana.

## C6. Compradores e vendas **[S]**

- Motivos de perda padronizados e obrigatórios em todos os fins de funil (hoje só parcial).
- **Pipeline visual (kanban) de propostas e processos** com SLA por etapa e alertas de estagnação.
- Simulador de comissão na proposta (o agente vê o impacto da negociação na sua comissão, usando a fórmula da Parte B §10.1).
- **Assinatura digital qualificada** (CMD/DocuSign) para contratos de mediação, propostas e CPCV — substitui parte do circuito de códigos por email **[S]**; o código simples mantém-se como fallback.
- Portal do processo para o cliente: checklist de documentos com upload direto pelo comprador/proprietário (alimenta o KYC do processo) **[S]**.

## C7. Gestão, qualidade e RH **[S]**

- Dashboards em tempo real (WebSocket) com os KPIs da Parte D; funil com drill-down até ao registo.
- QC como fluxo próprio: fila de visitas a auditar, formulário (os campos qc_* atuais), amostragem configurável, score de qualidade por agente.
- Gamificação: regras de pontos configuráveis (hoje espalhadas por 10 sítios), leaderboard em tempo real no dashboard, badges **[C]**.
- Escalas com self-service (troca entre agentes com aprovação) **[C]**.
- Gestão de metas por agente/equipa (angariações, visitas, propostas) com acompanhamento automático **[S]**.

## C8. Plataforma e dados **[M]**

- **Configuração sem deploy:** papéis funcionais (gestora CO, responsável financeiro, nº de emergência SMS), mapeamento agência↔distritos (incluindo exceções tipo Lourinhã→Leiria), pesos do matching, durações de janelas de PIN, limites de automação — tudo em backoffice.
- **RGPD:** registo de consentimento por canal com data/origem, anonimização/direito ao esquecimento, exportação de dados do titular, políticas de retenção.
- Importadores com pré-visualização e relatório de erros (leads, contactos, imóveis).
- Auditoria estruturada: quem viu/alterou/exportou o quê (evolução do chatter + document.security.log), com relatórios.
- API pública documentada (OpenAPI) para site, parceiros e integrações futuras.
- Qualidade de dados: painel de duplicados com merge assistido, validação de telefones/emails na entrada.

## C9. IA aplicada **[C]** (roadmap)

- Resumo automático de conversas WhatsApp/chamadas na ficha do contacto.
- Sugestão de resposta ao operador (com base nos guiões de approach/objeção atuais).
- Lead scoring preditivo (probabilidade de angariação/venda) para priorizar a fila do CO.
- Deteção de anomalias (comissões fora do padrão, visitas fantasma).

## C10. Configurabilidade total — Admin Console **[M]** ⭐

> Requisito do negócio: *"tudo tem de ser configurável pelo gestor/admin"* — vistas do assistente do CO, botões que cada grupo vê, comportamento de cada botão, e menus por grupo **e por utilizador**. Hoje isto exige alterar código.

O gestor/admin tem um **Admin Console** que controla o que cada pessoa vê e faz, sem deploy. Modelo de camadas de resolução: **Padrão do sistema → Perfil/Grupo → Utilizador (override)**. O override do utilizador vence sempre; o que não é definido herda do grupo; o que o grupo não define herda do padrão.

### C10.1 Menus e navegação configuráveis
- Editor visual da árvore de navegação: **por grupo** ("Grupo Operador CO vê: Fila do Dia, Prospectos, Agenda; não vê: Comissões, Relatórios") e **por utilizador** ("Utilizador X vê todos os menus; Utilizador Y não vê o menu Relatórios"), com um simples toggle de visibilidade por item de menu.
- Reordenar itens, agrupar, definir menu inicial (landing) por grupo/utilizador.
- Pré-visualização "Ver como este utilizador" (impersonation read-only) para validar antes de guardar.

### C10.2 Vistas e ecrãs configuráveis
- Por grupo/utilizador, definir a **vista por defeito** de cada listagem (tabela/kanban/mapa), **colunas visíveis e ordem**, filtros pré-aplicados e filtros guardados partilháveis.
- **Layout de formulários:** que separadores/secções/campos aparecem, quais são obrigatórios, quais são só-leitura — por grupo (ex.: o operador não edita a descrição pública do imóvel; o avaliador não vê o painel financeiro).
- Ex. concreto: **configurar a vista do assistente do CO** — que colunas tem a Fila do Dia, que informação do imóvel aparece na ficha do prospecto, que separadores estão visíveis.

### C10.3 Botões de ação configuráveis (visibilidade + comportamento)
- **Registo central de todos os botões/ações** do sistema (Não Atendida, Atendida, WhatsApp, Concluir, Descartar, Reservar, Aceitar Proposta, Rever Comissão…), cada um com:
  - **Visibilidade por grupo e por utilizador** (que grupos/utilizadores veem este botão).
  - **Permissão de execução** (ver ≠ poder clicar).
  - **Parâmetros de comportamento** expostos ao admin — o coração deste requisito. Exemplos:
    - **Botão "Não Atendido"**: `dias_reagendamento` (prospecto fica na vista do assistente durante o dia e ao final do dia reagenda para X dias — X **configurável**), `hora_reagendamento` (ex.: 09:00), `envia_sms` (sim/não) + template, `anti_duplo_clique_segundos`, `incrementa_contador`.
    - **Botão "Atendida"**: passos do wizard ativos, valores por defeito.
    - **Botão "Descartar"**: motivos disponíveis, se desativa o imóvel, se exige comentário.
    - **Botão "Concluir"**: se congela campos, estado destino.
  - Confirmação obrigatória sim/não, texto do botão e ícone/cor personalizáveis.
- Tudo o que hoje está hard-coded (os "+5 dias", "22h", "60 segundos", listas de motivos) passa a **parâmetro editável**, com valores por defeito iguais aos atuais para não quebrar a operação.

### C10.4 Grupos, permissões e papéis
- Criar/editar grupos, compor permissões finas (as ~40 da Parte B §2.4 + as novas), herança entre grupos (implied groups como hoje).
- Atribuir grupos e overrides a utilizadores; papéis funcionais (Gestora CO, responsável financeiro, nº de emergência SMS, coordenadora por empresa) como configuração.
- **Matriz de acesso** visual (grupos × entidades × operações CRUD) para auditar rapidamente "quem pode o quê".

### C10.5 Parâmetros de negócio e catálogos
- Catálogos editáveis: estados/estágios e suas cores, motivos (descarte, não-realização, perda), origens de lead, tipos de negócio, categorias de imóvel (por empresa), categorias de pontos, tipos de ato.
- Parâmetros globais: mapeamento agência↔distritos (com exceções tipo Lourinhã→Leiria), tolerância de orçamento do matching, pesos do score, durações de janela de PIN, limites/janelas das automações, IPs autorizados.

### C10.6 Governança da configuração
- Todas as alterações **auditadas** (quem, quando, antes→depois) e **versionadas** (poder reverter e responder "qual era a config em X data").
- Ambientes: rascunho → publicar (para o admin preparar mudanças e ativar de uma vez).
- Exportar/importar configuração entre ambientes (dev/staging/prod).
- Salvaguardas: não permitir remover o último admin nem desligar permissões críticas de si próprio.

---
---

# PARTE D — MIGRAÇÃO, KPIs, ACEITAÇÃO E RISCOS

## D1. Fases de migração

| Fase | Conteúdo | Paridade/novidade |
|---|---|---|
| **1 — Fundação** | RBAC + modelo `responsible_users`, entidades núcleo (Contact, Property, Prospect, Interaction), intake API, dedupe, importação do legado (odoo_sync), templates + log de envios | Parte B §2–§7 |
| **2 — Operação CO e visitas** | Fila do operador, botões de teleprospecção, automações de prospecção (motor C2), 3 tipos de visita + QC + notificações, agenda unificada | Parte B §5, §8 + C2 |
| **3 — Vendas e financeiro** | Propostas, processos, atos, comissões (fórmula exata), pontos, relatórios | Parte B §9–§11 |
| **4 — Recomendações e comunicação** | Motor de recomendação (C1), timeline omnicanal + WhatsApp nativo (C3), PWA do agente (C4) | Novo |
| **5 — Site e área de cliente** | API pública, área de cliente nova (auth moderna), angariação online, propostas online, publicação em portais (C5) | Parte B §15 + C5 |
| **6 — Cutover** | Operação paralela, migração por equipas/agências, desativação do Odoo | — |

Regra de convivência: durante as fases 1–5 o `odoo_sync` mantém sincronização bidirecional dos objetos ainda vivos no Odoo (mesmo padrão já usado no triomphe-whatsapp).

## D2. KPIs de sucesso

- Tempo médio até primeiro contacto; taxa de contacto efetivo por canal.
- Conversão por etapa: prospecto→visita, visita→angariação, angariação→proposta, proposta→fecho.
- Taxa de comparecimento em visitas; taxa de reagendamento/cancelamento.
- **Recomendações:** taxa de envio→visualização→interesse→visita; % de visitas comerciais originadas por recomendação.
- Dias em mercado por imóvel; desvio preço pedido vs. fechado.
- Taxa de duplicidade de lead; taxa de falha de automações; SLA de resolução por carteira.
- Adoção: % de relatórios de visita preenchidos no próprio dia (mobile), tempo médio de resposta do agente a recomendações.

## D3. Critérios de aceite (Release 1–2)

- Paridade funcional verificada contra a Parte B para os módulos migrados (catálogo de regras com testes de contrato).
- Permissões validadas por perfil/equipa/agência/carteira com testes automatizados (matriz da Parte B §3).
- Automações mínimas ativas e auditáveis (inserção, não atendida, follow-ups, pós-visita).
- Motor de recomendação a gerar matches para 100% dos compradores com perfil e área "Recomendações" utilizável pelo agente.
- Logs completos de comunicação e mudanças de estado.
- Dashboard com KPIs essenciais; importação do legado sem perdas (contagens reconciliadas).

## D4. Riscos e mitigações

| Risco | Mitigação |
|---|---|
| Regressão de regras implícitas do legado | Parte B como catálogo contratual + testes de contrato por regra |
| Replicar acoplamentos antigos | API de intake e serviços de domínio desacoplados; site só consome API pública |
| Segurança em canais públicos | Baseline obrigatório (rate limit, captcha, idempotência) em todos os endpoints |
| Migração de dados (IDs hard-coded, dados sujos) | Fase de limpeza + painel de duplicados + reconciliação de contagens |
| Adoção baixa da operação | Rollout por ondas/equipas, feature flags, formação, paridade de atalhos de trabalho (botões da teleprospecção) |
| Recomendações irrelevantes minam confiança | Pesos configuráveis, feedback loop de rejeições, revisão do agente antes do envio (modo assistido primeiro, automático depois) |
| Custo/entregabilidade SMS/WhatsApp | Push interno para notificações de colaboradores; SMS/WhatsApp reservados a clientes |

## D5. Backlog inicial

- **Must:** Fases 1–3 completas (paridade), motor de automações (C2), recomendação v1 (C1.1–C1.4), timeline omnicanal, PWA do agente, pesquisa global, configuração sem deploy, RGPD base.
- **Should:** Score configurável + sinais comportamentais (C1.5), recomendação inversa (C1.6), feeds de portais, AVM, assinatura digital, calendário sync, QC como fluxo, metas.
- **Could:** IA (C9), badges, escalas self-service, matching semântico.

## D6. Anexos — referências

- **Levantamento funcional detalhado (fonte da Parte B):** `LEVANTAMENTO_FUNCIONAL_CRM_ATUAL.md`
- Código legado: `Triomphe addons/remax-crm/rdx_remax_crm/` (crm.py, base.py, product.py, calendar.py, sale.py, hr.py, accounting.py, api.py, security/, data/, external_communications/, visits_notifications/, habitar/), `triomphe-website/triomphe_web/controllers/`, módulos auxiliares (`rdx_base_ext`, `rdx_hr_teams`, `rdx_sales_team_extended`, `rdx_pt_addresses_ext`, `rdx_product_images`, `rdx_olx_real_estates_crawler`).
- Arquitetura de referência do novo stack: `Triomphe addons/triomphe-whatsapp/` (Django apps + React/Vite).
